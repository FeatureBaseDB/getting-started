...
class StarTrace:
    ...
    def search(self, query):
        stargazer_field = open(self.get_path("stargazer.csv"), "w")
        language_field = open(self.get_path("language.csv"), "w")
        ...
        try:
            ...
            for i, repo in enumerate(search):
                ...
                try:
                    for lang in repo.get_languages().keys():
                        language_field.write("{lang_id},{project_id}\n".format(
                ...
                try:
                    for stargazer in repo.get_stargazers_with_dates():
                        stargazer_field.write("{stargazer_id},{project_id},{starred_at}\n".format(
            ...
            stargazer_field.close()
            language_field.close()
